...
<form action="{{route('submenus.store')}}" method="post" enctype="multipart/form-data">
	@csrf
	{!! input_field(["label"=>"Sm Name","name"=>"txtSm_name"]) !!}
	{!! select_field(["label"=>"Menu Id","name"=>"cmbMenuId","table"=>$menus]) !!}
	{!! input_field(["label"=>"Slug","name"=>"txtSlug"]) !!}
	{!! input_field(["label"=>"M Photo","name"=>"txtM_photo"]) !!}
